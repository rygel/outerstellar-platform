...
class AccountServiceTest {
    ...
    fun `deleteAccount blocks deleting the only admin`() {
        ...
        assertThrows<InsufficientPermissionException> { service.deleteAccount(adminUser.id, "correctpass") }
    ...
    fun `deleteAccount rejects wrong password`() {
        ...
        assertThrows<WeakPasswordException> { service.deleteAccount(testUser.id, "wrongpass") }
        verify(exactly = 0) { userRepository.deleteById(any()) }
    }
