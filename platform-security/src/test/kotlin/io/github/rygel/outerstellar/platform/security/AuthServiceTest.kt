...
class AuthServiceTest {
    ...
    fun `authenticate returns user on valid credentials`() {
        ...
        val result = authService.authenticate("testuser", "correctpass")
    ...
    fun `authenticate returns null for wrong password`() {
        ...
        val result = authService.authenticate("testuser", "wrongpass")
    ...
    fun `authenticate returns null for disabled user`() {
        ...
        val result = authService.authenticate("testuser", "anypass")
    ...
    fun `authenticate returns null for unknown user`() {
        ...
        val result = authService.authenticate("unknown", "anypass")
    ...
    fun `authenticate increments failed attempts on wrong password`() {
        ...
        val result = authService.authenticate("testuser", "wrong")
    ...
    fun `authenticate audits AUTHENTICATION_FAILED when user disabled`() {
        ...
        authService.authenticate("testuser", "anypass")
    ...
    fun `authenticate audits AUTHENTICATION_FAILED when account locked`() {
        ...
        authService.authenticate("testuser", "correctpass")
    ...
    fun `register throws on duplicate username`() {
        ...
        assertThrows<UsernameAlreadyExistsException> {
            authService.register("existing", "ValidP@ss1" + java.util.UUID.randomUUID().toString())
        }
    }
    ...
    fun `register throws on short password`() {
        ...
        assertThrows<WeakPasswordException> { authService.register("newuser", "short") }
    }
    ...
    fun `register throws on long password`() {
        ...
        assertThrows<WeakPasswordException> { authService.register("newuser", "A".repeat(129)) }
    }
    ...
    fun `register trims whitespace before validation`() {
        ...
        val result = authService.register("newuser", "  Validp@ss1  ")
