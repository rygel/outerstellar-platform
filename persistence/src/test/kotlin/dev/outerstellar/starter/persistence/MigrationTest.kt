...
class MigrationTest {
    ...
    fun `migrations are applied correctly`() {
        // Use an in-memory database for migration testing
        val jdbcUrl = "jdbc:h2:mem:migration_test;MODE=PostgreSQL;DB_CLOSE_DELAY=-1"
        ...
        dataSource.connection.use { conn ->
            val rs = conn.metaData.getTables(null, null, "MESSAGES", null)
            assertTrue(rs.next(), "MESSAGES table should exist")
            
            val rsSync = conn.metaData.getColumns(null, null, "MESSAGES", "SYNC_ID")
