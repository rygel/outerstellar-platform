package dev.outerstellar.platform.arch
...
class ArchitectureTest {
    ...
    fun `core should not depend on web or desktop or persistence`() {
        val importedClasses = ClassFileImporter().importPackages("dev.outerstellar.platform")
    ...
    fun `api-client should not depend on persistence, desktop, or web`() {
        ...
        // The api-client SyncService lives in dev.outerstellar.platform.sync.
        // This rule uses allowEmptyShould(true) so it passes without false positives.
        val importedClasses = ClassFileImporter().importPackages("dev.outerstellar.platform")
    ...
    fun `repository interfaces should reside in core`() {
        // Interfaces named *Repository should reside in dev.outerstellar.platform.persistence
        // (core).
        ...
        val rule =
            classes()
                ...
                .should()
                .resideInAPackage("dev.outerstellar.platform.persistence")
                .allowEmptyShould(true)
    ...
    fun `no cycles in service layer`() {
        // Checks that slices of dev.outerstellar.platform.<top-level-package> are free of cycles,
        // scoped to only the service-layer packages present in core.
        val importedClasses =
            ClassFileImporter()
                .importPackages(
                    "dev.outerstellar.platform.service",
                    "dev.outerstellar.platform.persistence",
                    "dev.outerstellar.platform.model",
                )

        val rule =
            SlicesRuleDefinition.slices()
                .matching("dev.outerstellar.platform.(*)..")
                .should()
                .beFreeOfCycles()
                .allowEmptyShould(true)
