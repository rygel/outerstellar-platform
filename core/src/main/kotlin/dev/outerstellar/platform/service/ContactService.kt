...
class ContactService(
    ...
    ): dev.outerstellar.platform.sync.SyncPullContactResponse {
        val changes = repository.findChangesSince(updatedAtEpochMs)
        val serverTimestamp = System.currentTimeMillis()
        return dev.outerstellar.platform.sync.SyncPullContactResponse(
    ...
    fun processPushRequest(
        request: dev.outerstellar.platform.sync.SyncPushContactRequest
    ): dev.outerstellar.platform.sync.SyncPushContactResponse {
        var applied = 0
        val conflicts = mutableListOf<dev.outerstellar.platform.sync.SyncContactConflict>()
        ...
        val process = {
            request.contacts.forEach { pushedContact ->
                ...
                if (
                    ...
                    conflicts.add(
                        dev.outerstellar.platform.sync.SyncContactConflict(
        ...
        return dev.outerstellar.platform.sync.SyncPushContactResponse(
