...
object ThemeCatalog {
    ...
    /**
     * Detects whether a theme is dark by computing the relative luminance
     * of its background color. Falls back to the `type` field when
     * the background color is absent or unparseable.
     */
    ...
    /**
     * Generates the basic CSS variables (preserving the original format)
     * that map each theme color key to a `--color-*` custom property.
     */
    ...
    fun toExtendedCssVariables(themeId: String): String {
        ...
        return buildString {
            append(":root[data-theme=\""); append(themeId); appendLine("\"] {")
    ...
    fun generateAllExtendedStyles(): String =
    ...
    /**
     * Adjusts the brightness of a hex color by the given [percent] (positive = lighter,
     * negative = darker). Returns the original value when parsing fails.
     */
    internal fun adjustBrightness(hexColor: String, percent: Int): String {
        val rgb = parseHexRgb(hexColor) ?: return hexColor
    ...
    /**
     * Computes the relative luminance of a hex color per WCAG 2.0.
     * Returns `null` when the string cannot be parsed.
     */
    internal fun relativeLuminance(hexColor: String): Double? {
        val rgb = parseHexRgb(hexColor) ?: return null
    ...
    private fun StringBuilder.cssVar(name: String, value: String) {
        append("    "); append(name); append(": "); append(value); appendLine(";")
    }

    /** Parses "#RGB" or "#RRGGBB" into an [IntArray] of [r, g, b]. */
