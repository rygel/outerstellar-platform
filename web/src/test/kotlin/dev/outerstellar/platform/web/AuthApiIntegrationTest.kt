...
class AuthApiIntegrationTest : H2WebTest() {
    ...
    fun `register api creates user and allows login`() {
        val registerLens =
            org.http4k.core.Body.auto<dev.outerstellar.platform.model.RegisterRequest>().toLens()
        val loginLens =
            org.http4k.core.Body.auto<dev.outerstellar.platform.model.LoginRequest>().toLens()
        val tokenLens =
            org.http4k.core.Body.auto<dev.outerstellar.platform.model.AuthTokenResponse>().toLens()
        ...
        val registerResponse =
            app(
                Request(POST, "/api/v1/auth/register")
                    .with(
                        registerLens of
                            dev.outerstellar.platform.model.RegisterRequest("api-user", "secret123")
                    )
            )
        ...
        val loginResponse =
            app(
                Request(POST, "/api/v1/auth/login")
                    .with(
                        loginLens of
                            dev.outerstellar.platform.model.LoginRequest("api-user", "secret123")
                    )
            )
