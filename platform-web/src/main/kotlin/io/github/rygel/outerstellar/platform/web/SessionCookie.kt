...
internal object SessionCookie {
    fun create(value: String, secure: Boolean, maxAgeSeconds: Long? = null): String {
        ...
        return "${RequestContext.SESSION_COOKIE}=$value; Path=/; HttpOnly; SameSite=Strict$securePart$maxAgePart"
    }

    fun clear(secure: Boolean): String {
        val securePart = if (secure) "; Secure" else ""
        return "${RequestContext.SESSION_COOKIE}=; Path=/; Max-Age=0; HttpOnly; SameSite=Strict$securePart"
    }
}
