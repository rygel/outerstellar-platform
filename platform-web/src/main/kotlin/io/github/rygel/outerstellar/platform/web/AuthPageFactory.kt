...
class AuthPageFactory(private val appleOAuthEnabled: Boolean = false) {
    ...
    fun buildAuthPage(ctx: RequestContext, shellRenderer: ShellRenderer): Page<AuthViewModel> {
        ...
        val shell = shellRenderer.shell(i18n.translate("web.nav.auth"), "/auth")
                    ...
                    tabs =
                        listOf(
                            AuthModeTab(
                                ...
                                shellRenderer.url("$formsUrl/sign-in?returnTo=$returnTo"),
                            ),
                            AuthModeTab(
                                "register",
                                i18n.translate("web.auth.register"),
                                shellRenderer.url("$formsUrl/register?returnTo=$returnTo"),
                            ),
                            AuthModeTab(
                                "recover",
                                i18n.translate("web.auth.recover"),
                                shellRenderer.url("$formsUrl/recover?returnTo=$returnTo"),
                            ),
                        ),
                    defaultFormUrl = shellRenderer.url("$formsUrl/sign-in?returnTo=$returnTo"),
                ),
        )
    }
    ...
    fun buildAuthForm(ctx: RequestContext, shellRenderer: ShellRenderer, mode: String): AuthFormFragment {
        ...
        return AuthFormFragment(
            ...
            submitUrl = shellRenderer.url("/auth/components/result?returnTo=$returnTo"),
    ...
    fun buildChangePasswordPage(shellRenderer: ShellRenderer): Page<ChangePasswordPage> {
        val i18n = shellRenderer.i18n
        val shell = shellRenderer.shell(i18n.translate("web.password.title"), "/auth")
        return Page(shell = shell, data = ChangePasswordPage(form = buildChangePasswordForm(shellRenderer)))
    }
    ...
    fun buildChangePasswordForm(shellRenderer: ShellRenderer): ChangePasswordForm {
        ...
        return ChangePasswordForm(
            ...
            submitUrl = shellRenderer.url("/auth/components/change-password"),
    ...
    fun buildResetPasswordPage(shellRenderer: ShellRenderer, token: String): Page<ResetPasswordPage> {
        val i18n = shellRenderer.i18n
        val shell = shellRenderer.shell(i18n.translate("web.reset.title"), "/auth")
        return Page(
            shell = shell,
            data =
                ResetPasswordPage(
                    ...
                    submitUrl = shellRenderer.url("/auth/components/reset-confirm"),
