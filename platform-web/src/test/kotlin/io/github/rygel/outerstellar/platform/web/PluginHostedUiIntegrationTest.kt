...
import io.github.rygel.outerstellar.platform.plugin.PluginRouteRegistration
...
class PluginHostedUiIntegrationTest : WebTest() {
    private fun repoQualityPlugin(
        ...
        object : PlatformPlugin {
            ...
            override val mode: PlatformMode = PlatformMode.PluginHostedApp
                    ...
                    homePath meta
                        ...
                        { request ->
                            ...
                            Response(Status.OK)
                                ...
                                .body(context.renderShell(shell, """<main id="plugin-home">RepoQuality</main>"""))
                        }
                return listOf(
    ...
    @Test
    fun `plugin hosted mode can own root ui by default`() {
        val app = buildApp(plugin = repoQualityPlugin(homePath = "/"))
        ...
        assertThat(body, containsSubstring("""id="plugin-home""""))
        assertThat(body, containsSubstring("""href="/""""))
        assertThat(body, !containsSubstring("""href="/repoquality""""))
    }

    @Test
    fun `plugin hosted mode does not mount default platform ui by default`() {
        val app = buildApp(plugin = repoQualityPlugin())
        val (token, _, _) = withAuthenticatedUser(role = "ADMIN")
        val authenticated = Cookie(RequestContext.SESSION_COOKIE, token)
    ...
    @Test
    fun `plugin hosted shell removes default platform chrome links`() {
        val app = buildApp(plugin = repoQualityPlugin())
        val (token, _, _) = withAuthenticatedUser(role = "ADMIN")
        val response =
            app(
    ...
    @Test
    fun `plugin hosted shell only exposes opted in platform links`() {
        val app =
            buildApp(
                plugin =
                    repoQualityPlugin(
