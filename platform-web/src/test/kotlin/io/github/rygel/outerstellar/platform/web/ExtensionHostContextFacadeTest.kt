...
class ExtensionHostContextFacadeTest {
    ...
    fun `users facade resolves current user from request context and delegates lookups`() {
        ...
        val baseRequest = Request(GET, "/extension/reports").cookie(Cookie(RequestContext.JWT_COOKIE, "jwt-token"))
    ...
    fun `security and notification facades delegate to narrowed host services`() {
        val userId = UUID.randomUUID()
        val notification = Notification(userId = userId, title = "Build ready", body = "Extension build completed")
        ...
        val expectedNotification =
            HostNotification(
                ...
                title = "Build ready",
                body = "Extension build completed",
                type = "info",
                read = false,
