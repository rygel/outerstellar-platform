...
class SyncApiIntegrationTest : WebTest() {
    ...
    fun `GET sync response is JSON with messages and serverTimestamp fields`() {
        ...
        assertNotNull(body.messages, "Pull response must have 'messages' field")
        assertNotNull(body.serverTimestamp, "Pull response must have 'serverTimestamp' field")
    }
    ...
    fun `POST sync push response contains conflicts field`() {
        ...
        val body = KotlinxSerialization.asA(response.bodyString(), SyncPushResponse::class)
        assertNotNull(body.conflicts, "Push response must include 'conflicts' field")
    }
    ...
    fun `GET sync-contacts response contains contacts and serverTimestamp fields`() {
        ...
        assertNotNull(body.contacts, "Contacts pull response must have 'contacts' field")
        assertNotNull(body.serverTimestamp, "Contacts pull response must have 'serverTimestamp' field")
    }
    ...
    fun `POST sync-contacts response contains appliedCount and conflicts`() {
        ...
        val body = KotlinxSerialization.asA(response.bodyString(), SyncPushContactResponse::class)
        assertNotNull(body.appliedCount, "Response must have 'appliedCount'")
        assertNotNull(body.conflicts, "Response must have 'conflicts'")
    }
}
