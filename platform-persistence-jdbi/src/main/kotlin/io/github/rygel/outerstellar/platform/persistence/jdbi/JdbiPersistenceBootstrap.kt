...
class JdbiPersistenceBootstrap : PersistenceBootstrap {
    override fun create(config: AppConfig, extensionMigrations: ExtensionMigrations?): PlatformPersistence =
        createPersistenceComponents(config = config, extensionMigrations = extensionMigrations)
}
