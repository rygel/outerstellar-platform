...
class SyncService(
    ...
    fun register(username: String, pass: String): AuthTokenResponse {
        val request =
            Request(POST, "$baseUrl/api/v1/auth/register").with(registerRequestLens of RegisterRequest(username, pass))
    ...
    fun markNotificationRead(notificationId: String) {
        val response = authenticatedRequest(Request(PUT, "$baseUrl/api/v1/notifications/$notificationId/read"))
        checkSessionExpired(response)
    }
